...
sub plot_core{
	...
	$self->plot_ruler( $ruler_y,$self->{left_margin}, $self->{left_margin} + $self->{tree_width}, $self->{tree_dist}, $self->{dist_type} ) if($self->{show_ruler} && $self->{tree_width} );
	...
	foreach my $p (@$array) {
		if($self->{show_W} && defined $p->{W}) {
			my $str = $p->{W};
			$str = sprintf("%.2f",$p->{W}) if($str=~/\d/);
			my $color = ($str > 1 || $str eq "infinite") ? "red" : "black";
			my $pop_str = "Ka/Ks = $p->{W}\\n";
			$pop_str .= "Ka = $p->{Dn}\\n";
			$pop_str .= "Ks = $p->{Ds}\\n"; 
			my $sw = ($str ne "infinite") ? textWidth($self->{font},$self->{fsize},$str) : textHeight($self->{font},$self->{fsize},"8");
			my $x = ($p->{x} - $self->{half_box} - $p->{P}->{x}  >= $sw) ? ($p->{P}->{x} + $p->{x} - $self->{half_box} - $sw)/2 : ($p->{x} - $self->{half_box} - $sw ) ;
			my $y = $p->{y}-4;
			if ($str eq "infinite") {
				$svg->text('x',$x,'y',$y,'fill',$color,'-cdata',"8",'font-size',$self->{fsize}, 'font-family',$self->{font},'onclick', "alert('$pop_str')", "transform"=>"rotate(-90,$x,$y)");
			}else{
				$svg->text('x',$x,'y',$y,'fill',$color,'-cdata',$str,'font-size',$self->{fsize}, 'font-family',$self->{font},'onclick', "alert('$pop_str')");
			}
