...
{	
	...
	my $self = Tree::nhx_svg->new(
		...
		width=>750,					height=>0, 
		left_margin=>30,			right_margin=>380,
		...
		align_left_margin=>380,		align_right_margin=>30,
...
sub draw_gene{
	my ($self, $xarray,$y,$strand,$color,$identity,$alignrate) = @_ ;
	...
	for (my $i=0; $i<@$pp; $i+=2) {
		...
		$svg->rect('x',$x_start, 'y',$y,'width',($x_end-$x_start || 1),'height',$exon_size,'fill',$color,'stroke',$color,'onclick',"alert('Identity: $identity\\nCutoff:   $alignrate')");
	}
...
sub plot_block{
	...
	foreach my $p (@$array) {
		if (!$p->{C} && defined $p->{block}) {
			...
			## draw aligned region
			$self->draw_gene($p->{block},$p->{y},$p->{strand},$col,sprintf("%.2f",$p->{identity}),sprintf("%.2f",$p->{alignrate}) ); 
